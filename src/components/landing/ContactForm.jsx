...
export default function ContactForm() {
  ...
  return (
    <section id="contact" style={{ padding: "7rem clamp(1.5rem,5vw,4rem)", background: "#FAFAF8", borderTop: "1px solid rgba(139,110,71,0.1)" }}>
      <div ref={ref} style={{ maxWidth: 680, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: "3rem", opacity: inView ? 1 : 0, transform: inView ? "none" : "translateY(20px)", transition: "all 0.7s ease" }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(2rem, 4vw, 2.5rem)", fontWeight: 400, color: "#2a2218", marginBottom: "1rem" }}>
            {c.title}
          </h2>
          <p style={{ fontFamily: "'Inter',sans-serif", color: "#888", fontSize: "0.95rem", lineHeight: 1.7 }}>{c.subtitle}</p>
        </div>
        <div style={{ opacity: inView ? 1 : 0, transform: inView ? "none" : "translateY(30px)", transition: "all 0.7s ease 0.15s" }}>
          {sent ? (
            ...
            <form onSubmit={handleSubmit} style={{ background: "#fff", border: "1px solid rgba(139,110,71,0.15)", borderRadius: 12, padding: "2.5rem" }}>
              ...
              <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.7rem", color: "#bbb", textAlign: "center", marginTop: "1rem" }}>{c.gdpr}</p>
            </form>
          )}
        </div>
      </div>
