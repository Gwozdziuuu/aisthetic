  ...
  useEffect(() => {
    if (inView && !started.current) {
      ...
      const timer = setInterval(() => {
        current += increment;
        if (current >= num) {setVal(num);clearInterval(timer);} else
...
export default function Stats() {
  ...
  return (
    <section style={{ padding: "5rem clamp(1.5rem,5vw,4rem)", background: "#FAFAF8", borderTop: "1px solid rgba(139,110,71,0.1)", borderBottom: "1px solid rgba(139,110,71,0.1)" }} className=" hidden">
